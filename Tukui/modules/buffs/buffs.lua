...
local function UpdateWeapons(button, slot, active, expiration)
	if not button.texture then
		...
		button.time:SetPoint("BOTTOM", 1, 0)
		button.time:SetFont(C.media.uffont, C.media.uffontsize, C.media.uffontflags)
				
		button.bg = CreateFrame("Frame", nil, button)
		button.bg:CreatePanel("ThickBorder", 30, 30, "CENTER", button, "CENTER", 0, 0)
...
local function CreateAuraHeader(filter, ...)
	...
	header.text = T.SetFontString(header, C.media.uffont, C.media.uffontsize, C.media.uffontflags)
...
ScanAuras(CreateAuraHeader("HELPFUL", "TOPRIGHT", Minimap, "TOPLEFT", -10, 0))
ScanAuras(CreateAuraHeader("HARMFUL", "BOTTOMRIGHT", Minimap, "BOTTOMLEFT", -10, 0))
