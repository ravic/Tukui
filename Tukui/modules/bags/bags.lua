...
local BAGSFONT = C["media"].uffont
local BAGSFONTSIZE = C["media"].uffontsize
local BAGSFONTFLAGS = C["media"].uffontflags
local ST_NORMAL = 1
local ST_FISHBAG = 2
local ST_SPECIAL = 3
local bag_bars = 0
...
function Stuffing:Layout(lb)
	...
	if lb then
		...
		f.gold:SetText(GetMoneyString(GetMoney(), BAGSFONTSIZE))
		f.editbox:SetFont(BAGSFONT, BAGSFONTSIZE, BAGSFONTFLAGS)
		f.detail:SetFont(BAGSFONT, BAGSFONTSIZE, BAGSFONTFLAGS)
		f.gold:SetFont(BAGSFONT, BAGSFONTSIZE, BAGSFONTFLAGS)
	...
	for _, i in ipairs(bs) do
		...
		if bag_cnt > 0 then
			...
			for j = 1, bag_cnt do
				...
				b.frame:Show()
				b.frame:SetTemplate("ThickBorder")
...
function Stuffing:PLAYER_ENTERING_WORLD()
	...
	ContainerFrame1:HookScript("OnShow", function(self)
		...
		keybackdrop:SetTemplate("ThickBorder")
