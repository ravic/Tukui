...
T.PP = function(p, obj)
	...
	if p>=1 and p<=3 and not left:IsShown() then left:Show() end
	if p>=4 and p<=6 and not right:IsShown() then right:Show() end
		...
		if min ~= max then
			...
			if unit == "player" and health:GetAttribute("normalUnit") ~= "pet" then
				if C["unitframes"].showtotalhpmp == true then
					health.value:SetFormattedText("|cff%02x%02x%02x%d|cff478942.%s|r", r * 255, g * 255, b * 255, floor(min / max * 100), min)
				else
					health.value:SetFormattedText("|cff%02x%02x%02x%d|cff478942.%s|r", r * 255, g * 255, b * 255, floor(min / max * 100), ShortValue(min))
				end
			elseif unit == "target" or (unit and unit:find("boss%d")) then
				health.value:SetFormattedText("|cff478942%s.|cff%02x%02x%02x%d|r", ShortValue(min), r * 255, g * 255, b * 255, floor(min / max * 100))
			...
			if unit == "player" and health:GetAttribute("normalUnit") ~= "pet" then
				health.value:SetFormattedText("|cff478942%s|r", min)
			elseif unit == "target" or unit == "focus"  or unit == "focustarget" or (unit and unit:find("arena%d")) then
				health.value:SetFormattedText("|cff478942%s|r", ShortValue(min))
			else
...
T.PostUpdateHealthRaid = function(health, unit, min, max)
	if not UnitIsConnected(unit) or UnitIsDead(unit) or UnitIsGhost(unit) then
		...
		if GetSubZoneText() == "Den of the Devourer" then --Special health text for Chimaeron fight
			if min <= 10000 then
				health.value:SetText("|cff559655"..min.."|r")
			else
				health.value:SetText(" ")
			end
		elseif min ~= max then
...
T.PostUpdatePower = function(power, unit, min, max)
	...
	elseif UnitIsDead(unit) or UnitIsGhost(unit) then
		...
		if min ~= max then
			if pType == 0 then
				if unit == "target" then
					power.value:SetFormattedText("|cff4da6ff%d.|cff4181ff%s|r", floor(min / max * 100), ShortValue(max - (max - min)))
				...
				elseif (unit and unit:find("arena%d")) or unit == "focus" or unit == "focustarget" then
					...
					if C["unitframes"].showtotalhpmp == true then
						power.value:SetFormattedText("|cff4181ff%s|cff4da6ff.%d|r", max - (max - min), floor(min / max * 100))
					else
						power.value:SetFormattedText("|cff4181ff%s|cff4da6ff.%d|r", max - (max - min), ShortValue(floor(min / max * 100)))
					end
				end
			else
			...
			if unit == "pet" or unit == "target" or unit == "focus" or unit == "focustarget" or (unit and unit:find("arena%d")) then
				power.value:SetFormattedText("|cff4181ff%s|r", ShortValue(min))
			else
				power.value:SetFormattedText("|cff4181ff%s|r", min)
			end
		end
	end
	if self.Name then
...
T.PostCreateAura = function(element, button)
	button:SetTemplate("ThickBorder")
	
	button.remaining = T.SetFontString(button, C["media"].uffont, C["media"].uffontsize, C["media"].uffontflags)
	...
	button.count:SetFont(C["media"].uffont, C["media"].uffontsize, C["media"].uffontflags)
...
T.PostUpdateAura = function(icons, unit, icon, index, offset, filter, isDebuff, duration, timeLeft)
	...
	if(icon.debuff) then
		...
		if (isStealable or ((TukuiDB.myclass == "MAGE" or TukuiDB.myclass == "PRIEST" or TukuiDB.myclass == "SHAMAN") and dtype == "Magic")) and not UnitIsFriend("player", unit) then
...
T.createAuraWatch = function(self, unit)
	...
	if (buffs) then
		for key, spell in pairs(buffs) do
			...
			icon:SetWidth(T.Scale(5))
			icon:SetHeight(T.Scale(5))
			...
			count:SetFont(C["media"].uffont, C["media"].uffontsize, C["media"].uffontflags)
